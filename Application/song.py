...
def normalizeAudioGain(audio, rgain, target=-10):
    ...
    factor = 10 ** (
        (-(target - rgain) / 10.0) / 2.0
    )  # Divide by 2 because we want sqrt (amplitude^2 is energy)
    audio *= factor
    return audio
...
class Song:
    def __init__(self, path_to_file):
        ...
        self.songBeginPadding = (
            0
        )  # Number of samples to pad the song with, if first segment index < 0
    ...
    def getSegmentType(self, dbeat):
        ...
        raise Exception(
            ...
            + str(dbeat)
            + ", should be between "
            + str(self.segment_indices[0])
            + ", "
            + str(self.segment_indices[-1])
        )
    ...
    def annotate(self):
        ...
        if not self.hasBeatAnnot() or not self.hasDownbeatAnnot():
            ...
            self.beats = (
                btracker.getBeats()
            )  # Do not keep the annotations in memory, so NOT self.beats = ...
            ...
            writeAnnotFile(
                ...
                ANNOT_BEATS_PREFIX,
                self.beats,
                {"tempo": self.tempo, "phase": self.phase},
            )
            ...
            writeBinaryAnnotFile(
                self.dir_, self.title, ANNOT_ODF_HFC_PREFIX, self.onset_curve
            )
        else:
            self.loadAnnotBeat()
        ...
        if not self.hasSegmentationAnnot():
            ...
            self.segment_indices, self.segment_types = structuralSegmentator.analyse(
                self
            )
            writeAnnotFile(
                self.dir_,
                self.title,
                ANNOT_SEGMENT_PREFIX,
                zip(self.segment_indices, self.segment_types),
            )
        else:
        ...
        if not self.hasKeyAnnot():
            ...
            writeCsvAnnotation(
                self.dir_, ANNOT_KEY_PREFIX, self.title, key + ":" + scale
            )
        ...
        if not self.hasSpectralContrastSummaryAnnot():
            ...
            fft = np.fft.fft
            pool = Pool()
            specContrast = SpectralContrast(
                frameSize=FRAME_SIZE, sampleRate=44100, numberBands=12
            )
            ...
            for i in segments_H:
                ...
                for frame in FrameGenerator(
                    self.audio[start_sample:end_sample],
            ...
            specCtrstDeltas = np.average(
                np.abs(calculateDeltas(pool["audio.spectralContrast"])), axis=0
            )
            specValleyDeltas = np.average(
                np.abs(calculateDeltas(pool["audio.spectralValley"])), axis=0
            )
            writeBinaryAnnotFile(
                self.dir_,
                self.title,
                ...
                np.concatenate(
                    (specCtrstAvgs, specValleyAvgs, specCtrstDeltas, specValleyDeltas)
                ),
            )
        ...
        if not self.hasSingingVoiceAnnot():
            ...
            def calculate_sing_features(audio):
                ...
                spectralContrast = SpectralContrast(
                    lowFrequencyBound=low_f, highFrequencyBound=high_f
                )
                pool = Pool()
            ...
            writeBinaryAnnotFile(
                self.dir_, self.title, ANNOT_SINGINGVOICE_PREFIX, is_singing
            )
    ...
    def loadAnnotBeat(self):
        beats_str, res_dict = loadAnnotationFile(
            self.dir_, self.title, ANNOT_BEATS_PREFIX
        )
    ...
    def loadAnnotOnsetCurve(self):
        self.onset_curve = loadBinaryAnnotFile(
            self.dir_, self.title, ANNOT_ODF_HFC_PREFIX
        )

    def loadAnnotDownbeat(self):
        downbeats_str, res_dict = loadAnnotationFile(
            self.dir_, self.title, ANNOT_DOWNB_PREFIX
        )
        self.downbeats = [float(b) for b in downbeats_str]

    def loadAnnotSegments(self):
        segment_annot_strs, segment_annot_dict = loadAnnotationFile(
            self.dir_, self.title, ANNOT_SEGMENT_PREFIX
        )
    ...
    def loadAnnotSegments_fixNegativeStart(self):
        ...
        if self.segment_indices[0] < 0:
            ...
            songBeginPaddingSeconds = (
                -self.segment_indices[0] * 4 * beat_length_s - self.downbeats[0]
            )
            self.songBeginPadding = int(songBeginPaddingSeconds * 44100)
            self.downbeats = [
                dbeat + songBeginPaddingSeconds for dbeat in self.downbeats
            ]
            self.downbeats = [
                i * 4 * beat_length_s for i in range(-self.segment_indices[0])
            ] + self.downbeats
            self.beats = [beat + songBeginPaddingSeconds for beat in self.beats]
            self.beats = [
                i * beat_length_s for i in range(int(self.beats[0] / beat_length_s))
            ] + self.beats
    ...
    def loadAnnotGain(self):
        self.replaygain = loadCsvAnnotationFile(self.dir_, ANNOT_GAIN_PREFIX)[
            self.title
        ]

    def loadAnnotKey(self):
        self.key, self.scale = loadCsvAnnotationFile(self.dir_, ANNOT_KEY_PREFIX)[
            self.title
        ].split(":")

    def loadAnnotSpectralContrast(self):
        self.spectral_contrast = loadBinaryAnnotFile(
            self.dir_, self.title, ANNOT_SPECTRALCONTRAST_PREFIX
        ).reshape((-1))

    def loadAnnotSingingVoice(self):
        self.singing_voice = loadBinaryAnnotFile(
            self.dir_, self.title, ANNOT_SINGINGVOICE_PREFIX
        ).reshape((-1))

    def calculateSongThemeDescriptor(self):
        features = self.spectral_contrast
        self.song_theme_descriptor = theme_pca.transform(
            theme_scaler.transform(features.astype("single").reshape((1, -1)))
        ).astype("single")
    ...
    def openAudio(self):
        ...
        if self.songBeginPadding > 0:
            self.audio = np.append(
                np.zeros((1, self.songBeginPadding), dtype="single"), self.audio
            )
