...
class StructuralSegmentator:
    def analyse(self, song):  # (audio_in, downbeats, tempo):
        ...
        selfsim_mfcc = cosine_similarity(
            np.array(pool["lowlevel.mfcc"]), np.array(pool["lowlevel.mfcc"])
        )
        ...
        peakDetection = PeakDetection(
            ...
            maxPeaks=100,
            orderBy="amplitude",
            range=len(novelty_product),
            maxPosition=len(novelty_product),
            threshold=threshold,
        )
        ...
        delta = 0.4
        for dbindex, downbeat in zip(
            range(len(downbeats)), np.array(downbeats) - downbeats[0]
        ):
            # Skip the peaks prior to the acceptance interval
            while peak_cur_s < downbeat - delta * downbeat_len_s and peak_idx < len(
                peaks_pos
            ):
                num_filtered_out += 1
                peak_idx += 1
        ...
        peaks_pos_modified = np.array(
            peaks_pos_modified
        )  # The modified positions of the peaks
        peaks_ampl_modified = np.array(
            peaks_ampl_modified
        )  # The amplitudes of the peaks
        peaks_pos_dbindex = np.array(
            peaks_pos_dbindex
        )  # The downbeat indices of the peaks
        ...
        highest_peaks_db_indices = (peaks_pos_dbindex[np.argsort(peaks_ampl_modified)])[
            -NUM_HIGHEST_PEAKS:
        ]
        highest_peaks_amplitudes = (
            peaks_ampl_modified[np.argsort(peaks_ampl_modified)]
        )[-NUM_HIGHEST_PEAKS:]
        distances = []  # Number of total peaks at multiple of 4
        distances_high = (
            []
        )  # Number of high peaks at multiple of 4 for each of the highest
        distances8 = []  # Number of total peaks at multiple of 8
        distances8_high = (
            []
        )  # Number of high peaks at multiple of 8 for each of the highest
        ...
        for i in range(8):
            ...
            distances8.append(
                sum(
                    [
                        h
                        for p, h in zip(
                            highest_peaks_db_indices, highest_peaks_amplitudes
                        )
                        if (p - i) % 8 == 0
                        and (
                            p + 1 not in highest_peaks_db_indices
                            or max(
                                highest_peaks_amplitudes[
                                    highest_peaks_db_indices == p + 1
                                ]
                            )
                            < 0.75 * h
                        )
                        and (
                            p + 2 not in highest_peaks_db_indices
                            or max(
                                highest_peaks_amplitudes[
                                    highest_peaks_db_indices == p + 2
                                ]
                            )
                            < 0.75 * h
                        )
            ...
            distances8_high.append(
                len([p for p in highest_peaks_db_indices if (p - i) % 8 == 0])
            )
        ...
        most_likely_8db_index = np.argmax(
            distances8 * np.array(distances8_high).astype(float)
        )
        ...
        segment_indices = [
            most_likely_8db_index
            if most_likely_8db_index <= 4
            else most_likely_8db_index - 8
        ]
        ...
        segment_indices.extend(
            [
                db
                for db in highest_peaks_db_indices
                if (db - most_likely_8db_index) % 8 == 0
            ]
        )  # Also have all important segments in there
        # Important events just before or after a phrase-aligned downbeat could indicate important events too
        segment_indices.extend(
            [
                db + 1
                for db in highest_peaks_db_indices
                if (db + 1 - most_likely_8db_index) % 8 == 0
            ]
        )
        segment_indices.extend(
            [
                db + 2
                for db in highest_peaks_db_indices
                if (db + 2 - most_likely_8db_index) % 8 == 0
            ]
        )
        ...
        adaptive_mean_rms = adaptive_mean(
            pool["lowlevel.rms"], 64
        )  # Mean of rms in window of [-4 dbeats, + 4 dbeats]
        mean_rms = np.mean(adaptive_mean_rms)
        adaptive_mean_odf = adaptive_mean(
            song.onset_curve, int((44100 * 60 / tempo) / 512) * 4
        )  # -4 dbeats, +4 dbeats
        ...
        def getSegmentType(dbindex):
            ...
            after_index = int(
                int((dbindex + 4) * 4 * 60.0 / tempo * 44100.0) / HOP_SIZE
            )  # 2 downbeats after the fade
            rms_after = adaptive_mean_rms[after_index] / mean_rms

            after_index = int(
                int((dbindex + 4) * 4 * 60.0 / tempo * 44100.0) / 512
            )  # 32 + downbeat index (running average range is [-32,32])
        ...
        for i in range(len(segment_indices) - 1):
            if (
                segment_indices[i + 1] - segment_indices[i] >= 32
            ):  # Segments are too far apart, so add some new ones
                previous_type = segment_types[i]
                for offset in range(
                    16, segment_indices[i + 1] - segment_indices[i], 16
                ):  # 16, 32, 48, ..., distance - 16
                    if (
                        getSegmentType(segment_indices[i] + offset) != previous_type
                    ):  # This offset introduces a new type of segment
