...
NUM_SONGS_IN_KEY_MINIMUM = (
    15
)  # Used for song selection: the minimum number of songs that the tracklister attempts to add to the next song candidate pool
NUM_SONGS_ONSETS = (
    6
)  # Used for song selection: the number of songs of which the ODFs have to be matched with the master
MAX_SONGS_IN_SAME_KEY = (
    6
)  # Used for song selection: how many times songs in the same key can be played after each other

ROLLING_START_OFFSET = (
    LENGTH_ROLLING_IN + LENGTH_ROLLING_OUT
)  # Start N downbeats after previous queue point.
...
def getHAfter(song, dbeat, n=1):
    ...
    options = [
        song.segment_indices[i]
        for i in range(len(song.segment_indices))
        if song.segment_types[i] == "H"
    ]
...
def getAllMasterSwitchPoints(song, fade_type):
    ...
    LH = [
        ...
        if types[i - 1] == "L" and types[i] == "H"
    ]
    HL = [
        indices[i]
        for i in range(1, len(indices))
        if types[i - 1] == "H" and types[i] == "L"
    ]
    ...
    if fade_type == TYPE_DOUBLE_DROP:
        ...
        cues = [
            i - LENGTH_ROLLING_OUT - LENGTH_ROLLING_IN - 1
            for i in HL
            if i - LENGTH_ROLLING_OUT > 0 and i <= indices[-1] - LENGTH_ROLLING_OUT
        ]
        ...
        L_fade_out = [
            min(
                ...
                (
                    getHAfter(song, i)
                    if not (getHAfter(song, i) is None)
                    else indices[-1]
                )
                - i,
            )
...
def getAllSlaveCues(song, fade_type, min_playable_length=32):
    ...
    LH = [
        ...
        if types[i - 1] == "L"
        and types[i] == "H"
        and indices[-1] - indices[i] > min_playable_length
    ]
    HL = [
        indices[i]
        for i in range(1, len(indices))
...
def getMasterQueue(song, start_dbeat, cur_fade_type):
    ...
    if P_ddrop > 0:
        ...
        if isDoubleDrop and len(cues) != 0:
            ...
            return (
                doubleDropDbeat - max_fade_in_len,
                TYPE_DOUBLE_DROP,
                max_fade_in_len,
                fade_out_len,
            )
    ...
    if True:  # Only reason this is here is to have the same indentation as above :)
        if True:
            ...
            cues = [c for c in cues if c[0] >= start_dbeat]
            if len(cues) == 0:
                cue, max_fade_in_len, fade_out_len = (
                    start_dbeat,
                    0,
                    min(LENGTH_CHILL_OUT, song.segment_indices[-1] - start_dbeat),
                )
            else:
    ...
    if fade_type == TYPE_DOUBLE_DROP or fade_type == TYPE_ROLLING:
        if len(cues) > 0:
            cue, fade_in_len = cues[
                np.random.randint(len(cues))
            ]  # -1 to switch just before drop
...
def calculateOnsetSimilarity(odf1, odf2):
    ...
    for i1 in range(len(odf1)):
        i2_center = int(
            i1 * slope + 0.5
        )  # Center of the [-N,N] beam around the diagonal
...
class TrackLister:
    def __init__(self, song_collection):
        ...
        self.crossfades = (
            None
        )  # Ordered list of crossfade objects: one less than number of songs
        self.song_collection = song_collection
        self.songsUnplayed = (
            song_collection.get_annotated()
        )  # Subset of song collection containing all unplayed songs
    ...
    def getFirstSong(self):
        ...
        self.songsUnplayed = (
            self.song_collection.get_annotated()
        )  # Subset of song collection containing all unplayed songs
    ...
    def chooseNewTheme(self, firstSong):
        ...
        for song in self.songsUnplayed:
            ...
            songs_distance_to_first_song.append(
                euclidean_distance(theme, firstSong.song_theme_descriptor)
            )
            song.close()
    ...
    def filterSongOptionsByThemeDistance(self, song_options, master_song):
        ...
        for song in song_options:
            ...
            dist_to_centroid = euclidean_distance(
                cur_theme_centroid, song.song_theme_descriptor
            )
        ...
        for i in song_options_closest_to_centroid[:NUM_SONGS_ONSETS]:
            ...
            logger.debug(
                ">> Theme difference {:20s} : {:.2f} ({} {})".format(
    ...
    def getBestNextSongAndCrossfade(
        self, master_song, master_cue, master_fade_in_len, fade_out_len, fade_type
    ):
        """
            ...
            for queue_slave_cur in cf.queue_2_options:
                ...
                for odf_start_dbeat in range(0, transition_len_corr, odf_segment_len):
                    odf_master = master_song.getOnsetCurveFragment(
                        ...
                        min(
                            master_cue_corr + odf_start_dbeat + odf_segment_len,
                    ...
                    odf_slave = s.getOnsetCurveFragment(
                        ...
                        min(
                            queue_slave_cur + odf_start_dbeat + odf_segment_len,
                            queue_slave_cur + transition_len_corr,
                        ),
                    )
                    onset_similarity = (
                        calculateOnsetSimilarity(odf_master, odf_slave)
                        / odf_segment_len
                    )
                ...
                singing_master = np.array(
                    master_song.singing_voice[
                        master_cue_corr : master_cue_corr + transition_len_corr
                    ]
                    > 0
                )
                ...
                if score < best_score and not singing_clash:
                    ...
                    best_score == np.inf and score < best_score_clash and singing_clash
                ):
        ...
        if (best_song.key, best_song.scale) not in closely_related_keys:
            ...
            if (shifted_key_up, shifted_scale_up) in closely_related_keys:
                ...
                self.semitone_offset = -1
            logger.debug(
                "Pitch shifting! {} {} by {} semitones".format(
        ...
        if (
            len(self.songsUnplayed) <= NUM_SONGS_IN_KEY_MINIMUM
        ):  # If there are too few songs remaining, then restart
