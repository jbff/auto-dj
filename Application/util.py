...
ANNOT_SUBDIR = (
    "_annot_auto/"
)  # Not _annot_... because we don't want to overwrite the existing (ground-truth) annotations!
...
def loadCsvAnnotationFile(directory, prefix):
    ...
    try:
        with open(
            os.path.join(directory, ANNOT_SUBDIR, prefix + ".csv"), "r+"
        ) as csvfile:
        ...
        logger.debug(
            "Csv annotation file not found, silently ignoring exception " + str(e)
        )
    return result
...
def loadAnnotationFile(directory, song_title, prefix):
    ...
    if os.path.exists(input_file):
        ...
        raise UnannotatedException(
            "Attempting to load annotations of unannotated audio" + input_file + "!"
        )
