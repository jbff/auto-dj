...
class ToolFixAnnotationApp(tk.Tk):
    def __init__(self, directories, *args, **kwargs):
        ...
        self.songSelector = tk.OptionMenu(
            self,
            self.varSongSelector,
            "",
            *tuple([s.title for s in self.songs]),
        ...
        self.b_downbeats_play = tk.Button(
            self, text="[ > ]", command=self.play_downbeats
        )
        self.b_downbeats_play.grid(row=2, column=1)
        self.b_downbeats_fix = tk.Button(
            self, text="[+1]", command=self.shift_downbeats
        )
        self.b_downbeats_fix.grid(row=2, column=2)

        tk.Label(self, text="Segments: ").grid(row=3)
        self.b_segments_shift_min8 = tk.Button(
            self, text="[-8]", command=lambda: self.shift_segments(-8)
        )
        self.b_segments_shift_min8.grid(row=3, column=1)
        self.b_segments_shift_min1 = tk.Button(
            self, text="[-1]", command=lambda: self.shift_segments(-1)
        )
        self.b_segments_shift_min1.grid(row=3, column=2)
        self.b_segments_shift_plus1 = tk.Button(
            self, text="[+1]", command=lambda: self.shift_segments(1)
        )
        self.b_segments_shift_plus1.grid(row=3, column=3)
        self.b_segments_shift_plus8 = tk.Button(
            self, text="[+8]", command=lambda: self.shift_segments(8)
        )
    ...
    def play_beats(self):
        ...
        song = self.song
        high_segments = [
            i for i, j in zip(song.segment_indices, song.segment_types) if j == "H"
        ]
        ...
        audioMarked = overlayAudio(
            audio,
            np.array(
                [b - start_beat for b in song.beats if b >= start_beat], dtype="single"
            ),
        )
        self.stream.write(
            audioMarked, num_frames=len(audioMarked), exception_on_underflow=False
        )
    ...
    def play_downbeats(self):
        ...
        audioMarked = overlayAudio(
            audio,
            np.array(
                [b - start_beat for b in song.downbeats if b >= start_beat],
                dtype="single",
            ),
        )
        self.stream.write(
    ...
    def shift_segment(self, idx, shift):
        ...
        if shift < 0:
            ...
            if (
                idx < len(self.song.segment_indices) - 1
                and self.song.segment_indices[idx + 1] > segment_index
            ):
                self.song.segment_indices[idx] = segment_index
            elif idx == len(self.song.segment_indices) - 1 and segment_index < len(
                self.song.downbeats
            ):
                self.song.segment_indices[idx] = segment_index
            else:
    ...
    def change_segment_type(self, idx):
        self.song.segment_types[idx] = (
            "H" if self.song.segment_types[idx] == "L" else "L"
        )
        segidx = self.song.segment_indices[idx]
        segtype = self.song.segment_types[idx]
        self.segment_buttons[4 * idx]["text"] = "[{}:{}]".format(segidx, segtype)
    ...
    def add_segment_buttons(self):
        ...
        for i, segidx, segtype in zip(
            range(len(self.song.segment_indices)),
            self.song.segment_indices,
            self.song.segment_types,
        ):
            # Add a label
            b = tk.Button(
                self,
                text="[{}:{}]".format(segidx, segtype),
                command=lambda i=segidx: self.play_segment(i),
            )
            self.segment_buttons.append(b)
            b.grid(row=4 + i, column=0)
            # Add some buttons to shift left and right
            b = tk.Button(
                self, text="[-1]", command=lambda i=i: self.shift_segment(i, -1)
            )
            self.segment_buttons.append(b)
            ...
            b = tk.Button(
                self, text="[L/H]", command=lambda i=i: self.change_segment_type(i)
            )
            self.segment_buttons.append(b)
    ...
    def save(self):
        ...
        writeAnnotFile(
            ...
            ANNOT_BEATS_PREFIX,
            song.beats,
            {"tempo": song.tempo, "phase": song.phase},
        )
        ...
        writeAnnotFile(
            ...
            zip(song.segment_indices, song.segment_types),
        )
