...
class BeatTracker:
    ...
    def __init__(
        ...
        self.fft_mag_1024_512 = (
            None
        )  # FFT magnitude on windows of 1024 samples, 512 hop size
        self.fft_phase_1024_512 = (
            None
        )  # FFT phase on windows of 1024 samples, 512 hop size
    ...
    def getPhase(self):
        ...
        if self.phase is None:
            raise Exception(
                "No phase detected yet, you must run the BeatTracker first!"
            )
        return self.phase
    ...
    def getBeats(self):
        ...
        if self.beats is None:
            raise Exception(
                "No beats detected yet, you must run the BeatTracker first!"
            )
        return self.beats
    ...
    def getOnsetCurve(self):
        ...
        if self.onset_curve is None:
            raise Exception(
                "No onset detection curve calculated yet, you must run the BeatTracker first!"
            )
    ...
    def run(self, audio):
        ...
        for bpm in valid_bpms:
            frames = (
                np.round(
                    np.arange(0, np.size(novelty_autocorr), numFramesPerBeat(bpm))
                ).astype("int")
            )[
                :-1
        ...
        for phase in valid_phases:
            ...
            frames = (
                np.round(
                    np.arange(phase_frames, np.size(novelty_hwr), numFramesPerBeat(bpm))
                ).astype("int")
            )[
                :-1
            ]  # Discard last value to prevent reading beyond array (last value rounded up for example)
        ...
        beats = np.arange(phase, (np.size(audio) / 44100) - spb + phase, spb).astype(
            "single"
        )
