...
def time_stretch_hpss(audio, f):
    ...
    stft_harm, stft_perc = decompose.hpss(
        stft, kernel_size=31
    )  # original kernel size 31

    # OLA the percussive part
    y_perc = librosa.util.fix_length(
        core.istft(stft_perc, dtype=audio.dtype), len(audio)
    )
    ...
    y_harm = librosa.util.fix_length(
        core.istft(stft_harm, dtype=audio.dtype), len(audio)
    )
    y_harm = librosa.util.fix_length(
        time_stretch_sola(core.istft(stft_harm, dtype=audio.dtype), f, wsola=True),
        len(y_perc),
    )
...
def time_stretch_sola(audio, f, sample_rate=44100, wsola=False):
    ...
    frame_len_1 = (
        4410 if wsola else 4410 / 8
    )  # Length of a fragment, including overlap at one side; about 100 ms; shouldn't be longer because length of a 16th note is about this period at 175 BPM: otherwise it won't be possible to copy without doubling transients
    overlap_len = frame_len_1 / 8  # About 8 ms
    frame_len_2 = (
        frame_len_1 + overlap_len
    )  # Length of a fragment, including overlap at both sides
    frame_len_0 = (
        frame_len_1 - overlap_len
    )  # Length of a fragment, excluding overlaps (unmixed part)
    next_frame_offset_f = frame_len_1 / f  # keep as a float to prevent rounding errors
    next_frame_offset = int(
        next_frame_offset_f
    )  # keep as a float to prevent rounding errors
    ...
    def find_matching_frame(frame, theor_center):
        ...
        correlation = signal.fftconvolve(
            audio[cur_win_min : cur_win_max + len(frame)], frame[::-1], mode="valid"
        )  # Faster than np.correlate! cf http://scipy-cookbook.readthedocs.io/items/ApplyFIRFilter.html
    ...
    for out_ptr in range(0, num_frames_out * frame_len_1, frame_len_1):
        ...
        output[
            out_ptr : out_ptr + len(frame_to_copy)
        ] = (
            frame_to_copy
        )  # intermediate step to prevent mismatch between out : out+len and in:in+len'
        ...
        if in_ptr + frame_len_1 > audio.size:
            ...
            output[
                out_ptr + frame_len_0 : out_ptr + frame_len_0 + len(frame_to_copy)
            ] = frame_to_copy
            return output
        ...
        frame_to_match = audio[
            in_ptr + frame_len_0 : in_ptr + frame_len_0 + frame_len_1
        ]
        if wsola:
            match_ptr = find_matching_frame(
                frame_to_match, int(in_ptr_th_f + next_frame_offset_f) - overlap_len
            )
        else:
